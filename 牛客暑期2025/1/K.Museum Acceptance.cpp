...
int main(){
	...
	lst.init(2*m);
	fill(bel+1,bel+1+2*m,0);
	fill(siz+1,siz+1+2*m,0);
	...
	for(int i=1;i<=2*m;i++){
		...
		for(int cur=i;!bel[cur];){
			bel[cur]=cnt,siz[cnt]++;
			int v=ev[cur],eid=(cur-1)%m+1,ne=0;
			for(size_t j=0;j<e[v].size();j++){
				if((e[v][j].second-1)%m+1==eid){
					ne=get_eid(e[v][(j+1)%e[v].size()],0);
					break;
				}
			}
			assert(ne);
